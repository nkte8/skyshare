...
- OGPカードの生成とURL埋め込みの概念を切り離しました。1.1.0から挙動が若干変わります。以下のような動作になるように改善しました。
  - **OGPを生成しないオプションのON/OFFにかかわらず、画像が添付されていない場合はリンクカードの埋め込みを試みます** 。
    - 複数のURLが記載されている場合の動作は、最も最後に検出したURLを用います（v1.1.0と同様の動作）
  - **OGPを生成しないオプションのON/OFFにかかわらず、 画像が添付されている場合はリンクカードの埋め込みを試みません。**
    - 画像の添付とURLの付与は共存可能ですが、画像の添付とリンクカードの埋め込みは共存できないためです。
  - **OGPを生成しないオプションがONであり、画像が添付されている場合、OGPジェネレータを実施しません。**
